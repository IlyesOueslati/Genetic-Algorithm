...
class Individual():
  ...
  def fitness(self):
    score = 0 # price of all products
    sum_spaces = 0 # sum of the space for all products
    for i in range(len(self.chromosome)): # Check the chromosome
      if self.chromosome[i] == '1':
        score += self.prices[i] # add the value of the product
        sum_spaces += self.spaces[i]
    if sum_spaces > self.space_limit: # the sum must be less than the space limit
      score = 1
...
if __name__ == '__main__':
        products_list = []
        connection = pymysql.connect(host='localhost', user='root', passwd='23011997', db='products')
